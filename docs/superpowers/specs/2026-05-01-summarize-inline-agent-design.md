...
| 维度 | 合并前 | 合并后 |
|------|--------|--------|
| 输入 | spec + roadmap + toolchain + git log + diff + pytest + web search | 完全相同 |
...
以 agent 的 Output_Format 为准（含完整 frontmatter 模板和所有 section 标题），合并 skill 中 Task() 格式提示的补充细节（如 frontmatter.previous 字段说明）。10-section 报告 + frontmatter，≤ 200 行文本（mermaid 除外）。
