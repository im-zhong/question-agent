...
| 阶段 | 候选技术 | 支撑功能 | 触发条件 |
|------|---------|---------|---------|
| Phase 2 | ChromaDB（嵌入式向量库） | 知识点相似检索、已出题目去重检测、常错点模式聚类 | SQLite 关键词匹配无法满足语义相似度检索时 |
| Phase 2 | Streamlit | 编辑/教研人员交互式出题面板（知识点选择、难度调节、题目预览） | 需要向非技术用户演示或小范围试用时 |
| Phase 2 | GLM-5V（视觉模型） | 图像密集型教材页面的图文混排理解、公式/图表识别 | PDF 文本提取对公式、插图、表格混排页面失败率高时 |
| Phase 3 | PostgreSQL + pgvector | 多用户并发下的知识点库和题目库管理、向量检索生产化 | 从单机演示转向多编辑协作场景时 |
