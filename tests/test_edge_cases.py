...
@pytest.mark.asyncio
async def test_health_post_returns_405(client: AsyncClient):
    resp = await client.post("/health")
    assert resp.status_code == 405


@pytest.mark.asyncio
async def test_health_head_returns_200(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_extract_text_gbk_encoding(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_extract_text_with_bom(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_extract_text_binary_content_disguised_as_text(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_extract_text_one_byte_file(client: AsyncClient):
    resp = await client.post(
        "/extract",
...
@pytest.mark.asyncio
async def test_extract_pdf_empty_pdf(client: AsyncClient):
    zero_page_pdf = (
        b"%PDF-1.4\n"
...
@pytest.mark.asyncio
async def test_extract_pdf_large_valid_pdf(client: AsyncClient):
    pdf = FPDF()
